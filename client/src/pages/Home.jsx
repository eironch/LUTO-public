...
function Home(p) {
  ...
  function getFeedRecipes() {
    axios
      .get("http://localhost:8080/feed-recipes", {
    ...
    <div className="scrollable-div overflow-y-scroll">
      ...
      <div className="pr-0 flex flex-col gap-3 h-svh bg-transparent">
        <div className="flex flex-col gap-3 p-3 pr-0">
          ...
          <div
            className="grid w-full gap-3 h-16"
          ...
          {/* content */}
          <div
            className="grid w-full gap-3 h-full"
            style={{ gridTemplateColumns: "repeat(15, minmax(0, 1fr))" }}
          >
            <div className="col-span-2"></div>
        ...
        {(currentTab === "Home" ||
          currentTab === "Search" ||
          currentTab === "Settings") && (
          <SearchBar currentTab={currentTab} setCurrentTab={setCurrentTab} />
        )}
      </div>
    </div>
  );
