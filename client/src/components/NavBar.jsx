...
function NavBar(p) {
  return (
    <div
      className={`${
        p.currentTab === "Home" && "pr-0"
      } fixed flex flex-col gap-3 p-3 w-full h-svh pointer-events-none`}
    >
      {/* above the navbar */}
      ...
      {/* navbar */}
      <div
        className="grid gap-3 w-full h-16 pointer-events-none"
      ...
      >
        ...
        {p.currentTab === "Profile" && (
          <button
            ...
            } col-span-2 bg-zinc-900 pointer-events-auto flex flex-row w-full rounded-3xl hover:bg-zinc-500 hover:shadow-sm hover:shadow-zinc-950`}
            onClick={() => {
        ...
        <div
          className={`${
            p.currentTab !== "Home" && "bg-zinc-900"
          } rounded-3xl flex items-center justify-center`}
          style={{
            gridColumn: p.currentTab === "Home" ? "span 11" : "span 13",
          }}
        >
          {p.currentTab !== "Home" && (
            <img
              className="p-3 w-48 fixed left-1/2 transform -translate-x-1/2"
              src={Logo}
              alt=""
            />
          )}
